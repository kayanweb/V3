...
import {
  Users, Shield, Search, Loader2, RefreshCw, CheckCircle2, XCircle, AlertTriangle,
} from 'lucide-react'
...
const STATUS_CONFIG: Record<UserStatus, { label: string; labelEn: string; className: string }> = {
  ...
  suspended: { label: 'موقوف',  labelEn: 'Suspended', className: 'bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-400' },
}
...
export function UsersTab() {
  ...
  const [, setDepartments] = React.useState<DepartmentRecord[]>([])
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          { label: isAr ? 'موقوف' : 'Suspended', value: suspendedCount, icon: AlertTriangle,   color: 'text-amber-600' },
          ...
          {loading ? (
            ...
            <div className="space-y-2 max-h-[480px] overflow-y-auto">
              {filtered.length === 0 ? (
                ...
                filtered.map((u) => {
                  const status = (u.status ?? 'inactive') as UserStatus
                  ...
                  return (
                    <div key={u.id} className="flex flex-col sm:flex-row sm:items-center gap-3 p-3 rounded-lg border hover:bg-muted/30 transition-colors">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        {/* Role selector */}
                        <Select value={primaryRoleId} onValueChange={(v) => handleRoleChange(u.id, v)} disabled={isSaving}>
                        ...
                        {status === 'active' ? (
                          <Button size="sm" variant="ghost" className="h-7 text-xs text-amber-600 hover:text-amber-700 px-2 gap-1" disabled={isSaving} onClick={() => handleStatusChange(u.id, 'suspended')}>
